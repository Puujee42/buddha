      ...
      >
        ...
        <section className="relative h-screen flex items-center justify-center overflow-hidden">
          <motion.div style={{ y: yVideo }} className="absolute inset-0 z-0">
             ...
             <video autoPlay loop muted playsInline className={`w-full h-full object-cover transition-all duration-1000 ${isNight ? "opacity-30 brightness-75 contrast-125" : "opacity-60"}`}>
          ...
          <div className="relative z-30 text-center px-6 max-w-6xl mx-auto">
             <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.5 }} style={{ y: textY }}>
               <div className="flex justify-center mb-8">
                 <DharmaWheel className={`w-20 h-20 animate-[spin_60s_linear_infinite] transition-colors duration-1000 ${isNight ? "text-cyan-400 drop-shadow-[0_0_20px_#50F2CE60]" : "text-amber-500"}`} />
               </div>

               <h1 className="text-6xl md:text-[9rem] font-celestial font-black tracking-tighter mb-8 drop-shadow-[0_0_30px_rgba(255,255,255,0.1)]">
                 {content.heroTitle}
               </h1>
               
               <p className={`text-xl md:text-3xl font-serif italic font-light py-8 px-12 border-y transition-colors duration-1000 ${isNight ? 'border-cyan-400/20 text-cyan-200' : 'border-amber-500/30 text-amber-50'}`}>
            ...
            <div className="container mx-auto px-6 lg:px-24">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
                    ...
                    <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ margin: "-100px" }} className="space-y-12">
                        ...
                        <div className={`w-32 h-[1px] bg-linear-to-r from-current to-transparent ${isNight ? "text-[#C72075]" : theme.accent}`} />
                    ...
                    <div className="relative group perspective-2000">
                         <div className={`absolute inset-0 blur-[120px] opacity-20 group-hover:opacity-40 transition-all duration-1000 ${isNight ? "bg-[#C72075]" : "bg-amber-500"}`} />
                         
                         <motion.div initial={{ rotateY: 15, opacity: 0 }} whileInView={{ rotateY: 0, opacity: 1 }} className={`relative rounded-t-[25rem] rounded-b-[2rem] overflow-hidden border-[1px] backdrop-blur-md shadow-2xl transition-all duration-1000 ${isNight ? 'border-cyan-400/20' : theme.cardBorder}`}>
                            <img 
                              src="https://images.unsplash.com/photo-1601633596408-f421f28b499f?q=80&w=2574&auto=format&fit=crop" 
                              alt="Temple" 
                              className={`w-full h-[750px] object-cover scale-110 group-hover:scale-100 transition-transform duration-[4s] ${isNight ? "grayscale-[0.5] brightness-75 contrast-125 saturate-50" : "sepia-[0.2]"}`}
                            />
        ...
        <section className={`relative py-48 transition-colors duration-1000 ${theme.bgSec} backdrop-blur-xl`}>
            <div className="container mx-auto px-6 relative z-10">
                <div className="text-center mb-32 max-w-4xl mx-auto">
                    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}>
                       <Flower className={`w-12 h-12 mx-auto mb-8 animate-pulse ${isNight ? 'text-cyan-400' : theme.accent}`} />
  ...
  return (
    ...
    >
      <motion.div 
        ...
        className={`relative p-10 h-[500px] flex flex-col transition-all duration-1000 shadow-2xl rounded-sm border-2 group backdrop-blur-md ${theme.cardBg} ${theme.cardBorder}`}
      >
        {/* Zodiac Glow Interaction */}
        {isNight && (
          <div className="absolute inset-0 bg-radial-[circle_at_center] from-[#C72075]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
        )}
        ...
        <div className="relative z-10 h-full flex flex-col">
          <div className="flex justify-between items-start mb-12">
             <span className={`text-4xl font-serif font-black transition-colors ${isNight ? "text-cyan-900 drop-shadow-[0_0_8px_rgba(80,242,206,0.3)]" : "text-amber-100"}`}>{number}</span>
             <div className={`p-4 rounded-full border transition-all duration-500 group-hover:-translate-y-2 ${isNight ? "bg-cyan-950 border-cyan-500 text-cyan-400 shadow-[0_0_15px_#50F2CE40]" : "bg-white border-amber-200 text-amber-600"}`}>
          ...
          <div className={`w-12 h-1 mb-8 bg-linear-to-r from-current to-transparent ${isNight ? "text-[#C72075]" : theme.accent}`} />
          
          <p className={`text-sm leading-relaxed font-medium transition-colors opacity-80 italic`}>
            "{desc}"
          </p>

          <div className="mt-auto flex justify-center opacity-10 group-hover:opacity-60 transition-opacity">
            <Star size={20} className={isNight ? "text-cyan-400" : "text-amber-500"} />
          </div>
        </div>

        {/* Cosmic Shimmer Reflection */}
        <div className={`absolute inset-0 bg-linear-to-tr from-transparent ${isNight ? 'via-cyan-400/10' : 'via-white/5'} to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-[1.5s] pointer-events-none`} />
      </motion.div>
    </motion.div>
  );
}
