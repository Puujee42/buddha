      ...
      <div className="w-full lg:w-1/2 relative flex items-center justify-center p-6 sm:p-12">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3 mb-8 p-1 bg-[#451a03]/5 rounded-2xl">
            <button onClick={() => setRole("client")} className={`flex flex-col items-center justify-center py-4 rounded-xl transition-all duration-300 gap-2 ${role === "client" ? "bg-white shadow-md text-[#D97706]" : "text-[#78350F]/60 hover:bg-white/50"}`}>
              <User size={20} />
              <span className="text-xs font-bold uppercase tracking-wider">{content.roleClient}</span>
            </button>
            <button onClick={() => setRole("monk")} className={`flex flex-col items-center justify-center py-4 rounded-xl transition-all duration-300 gap-2 ${role === "monk" ? "bg-white shadow-md text-[#D97706]" : "text-[#78350F]/60 hover:bg-white/50"}`}>
              <ScrollText size={20} />
          ...
          <ClerkLoaded>
            <div className="space-y-6">
              ...
              <div className="text-center">
                <span className="text-xs font-sans text-[#D97706]/60 font-bold uppercase tracking-widest block mb-1">{content.or}</span>
              </div>

              <SignInButton mode="modal" forceRedirectUrl="/dashboard">
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="w-full py-4 rounded-2xl border-2 border-[#451a03]/20 bg-transparent hover:border-[#451a03] hover:bg-[#451a03]/5 transition-all flex items-center justify-center gap-2 text-[#451a03] font-bold group cursor-pointer">
          ...
          <div className="mt-10 text-center border-t border-[#D97706]/10 pt-6">
             <p className="text-[#78350F]/60 text-xs font-sans">
                {content.agreeText}
                <Link href="/" className="font-bold text-[#D97706] hover:underline ml-1">{content.eightfoldPath}</Link>
             </p>
          </div>
        </motion.div>
      </div>
